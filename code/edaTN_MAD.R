...
data$naRange %>%   
  ...
  labs(x = "Date", y = "Variable", color = "Range")

data$naRange %>% 
  pivot_wider(names_from = "range", values_from = date) %>% 
  filter(year(endDate) == 2021) %>% 
  pivot_longer(
    c(endDate, startDate), names_to = "range", values_to = "date"
  ) %>% 
  ggplot(
...
distanceFromGoldenGate %>% 
  ggplot() +
  geom_sf() +
  # geom_sf(data = data$cdecMetadata %>% filter(station %in% (
  #   what2 %>% filter(is.na(SubRegion)) %>% 
  #     pull(stationId) %>% unique()
  # )), aes(color = SubRegion), size = 2) +
...
# Increasing timeout in R since some of these CDEC data download can be large
options(timeout = 800) # default is 60 seconds
cdec <- list()

# Pulling in cdec data now
cdec <- lapply(c("TEMPERATURE", "ELEC.* CON.*", "TURBIDITY", "OXYGEN"),
               function(x) {
                 deltadata:::cdecMetadata %>% 
                   filter(gage %in% (
                     data$cdecMetadata %>% 
                       filter(!is.na(SubRegion)) %>% 
                       pull(station)
                   ),
                          grepl(x, sensorDescription)) %>% 
                   separate_wider_delim("dataAvailable", 
                                        delim = " to ", 
                                        names = c("dateStart", "dateEnd")) %>% 
                   mutate(dateStart = mdy(dateStart),
                          dateEnd = if_else(dateEnd == "present", Sys.Date(), mdy(dateEnd)),
                          duration = factor(duration, level = c("event", "hourly", "daily"))) %>% 
                   filter(dateStart <= min(data$final$collection_date, na.rm = T)) %>% 
                   group_by(gage) %>% 
                   slice_min(duration) %>% 
                   ungroup()
               }) %>% 
  setNames(c("TEMPERATURE", "ELEC.* CON.*", "TURBIDITY", "OXYGEN"))
...
data$cdec <- lapply(cdec, function(variable) {
  variable %>% 
    group_by(duration, sensorNumber) %>% 
    group_split() %>% 
    lapply(., function(df, maxAttempts = 3) {
      
      for (attempt in 1:maxAttempts) {
        result <- tryCatch({
          pullCDEC(
            station = unique(df$gage),
            sensor = unique(df$sensorNumber),
            duration = as.character(unique(df$duration)),
            dateStart = min(data$final$collection_date, na.rm = TRUE),
            dateEnd = max(data$final$collection_date, na.rm = TRUE),
            temperatureUnits = "C"
          )
        }, error = function(e) {
          if (grepl("cannot open the connection", e) && attempt < maxAttempts) {
            cat(sprintf("Attempt %d: Connection timed out, retrying\n", attempt))
            return(NULL)
          } else if (attempt == maxAttempts || !grepl("cannot open the connection", e)) {
            return(data.frame(
              stationId = unique(df$gage),
              sensorNumber = unique(df$sensorNumber),
              duration = as.character(unique(df$duration))
            ))
          }
        })
...
data$cdecDaily <- lapply(data$cdec, function(data) {
  data %>%
    bind_rows() %>%
    group_by(obsDate, stationId, sensorNumber) %>%
    summarise(value = mean(value, na.rm = T), .groups = "drop") %>%
    left_join(
      deltadata:::cdecMetadata %>% distinct(sensorDescription, sensorNumber),
      by = "sensorNumber"
    )
})
...
data$cdecSpatialAggregation <- data$cdecLessOutliers %>% 
  ...
  mutate(
    variable = case_when(
      sensorNumber %in% c(25, 146) ~ "temperatureWater", # find this info from 'cdec'
      sensorNumber %in% c(4) ~ "temperatureAir",
      sensorNumber %in% c(100) ~ "conductivityTop",
      sensorNumber %in% c(102) ~ "conductivityBottom",
      sensorNumber %in% c(27) ~ "turbidityNtu",
      sensorNumber %in% c(221) ~ "turbidityFnu",
      sensorNumber %in% c(61) ~ "dissolvedOxygen"
    )
  ) %>% 
  group_by(variable, obsDate, SubRegion, Region) %>% 
  summarise(valueSpatial = mean(value, na.rm = T), .groups = "drop")
...
missingDays <- data$cdecSpatialAggregation %>% 
  mutate(SubRegion = factor(SubRegion, levels = distanceFromGoldenGate$SubRegion)) %>% 
  complete(obsDate = seq(min(obsDate), max(obsDate), by = "day"), 
           variable, SubRegion,
           fill = list(value = NA)) %>% 
  group_by(variable, SubRegion) %>% 
  summarise(
    totalDays = n(),
    missingDays = sum(is.na(valueSpatial)),
    firstDate = min(obsDate[!is.na(valueSpatial)]),
    lastDate = max(obsDate[!is.na(valueSpatial)]),
    daysInRange = as.integer(difftime(lastDate, firstDate, units = "days")) + 1,
    missingDaysInRange = daysInRange - sum(!is.na(valueSpatial)),
    .groups = "drop"
  ) %>% 
  mutate(missingDays = ifelse(missingDays == totalDays, NA, missingDays))

# Temperature
distanceFromGoldenGate %>% 
  left_join(
    missingDays %>% 
      select(variable, SubRegion, missingDays),
    by = c("SubRegion")
  ) %>% 
  pivot_wider(names_from = variable, values_from = missingDays) %>%
  pivot_longer(cols = c(temperatureAir, temperatureWater,
                        # conductivityBottom, conductivityTop,
                        # turbidityFnu, turbidityNtu,
                        # dissolvedOxygen
  ), 
  names_to = "variable", 
  values_to = "missingDays") %>%
  ggplot() +
  geom_sf(aes(fill = missingDays)) +
  geom_sf(data = data$cdecMetadata %>% 
            right_join(data$cdecDaily$TEMPERATURE %>% 
                         mutate(variable = ifelse(grepl("AIR", sensorDescription), 
                                                  "temperatureAir", "temperatureWater")) %>% 
                         distinct(stationId, variable), by = c("station" = "stationId")),
          color = "khaki",
          size = 2) +
  facet_wrap(~variable) +
  # scale_fill_gradient(low = "navy", high = "cyan", na.value = "gray") +
  labs(fill = "Missing Days")
...
distanceFromGoldenGate %>% 
  ...
  geom_sf(aes(fill = missingDays)) +
  geom_sf(data = data$cdecMetadata %>% 
            right_join(data$cdecDaily$`ELEC.* CON.*` %>% 
                         mutate(variable = ifelse(grepl("BOTTOM", sensorDescription), 
                                                  "conductivityBottom", "conductivityTop")) %>% 
                         distinct(stationId, variable), by = c("station" = "stationId")),
          color = "khaki",
          size = 2) +
...
distanceFromGoldenGate %>% 
  ...
  geom_sf(aes(fill = missingDays)) +
  geom_sf(data = data$cdecMetadata %>% 
            right_join(data$cdecDaily$TURBIDITY %>% 
                         mutate(variable = ifelse(grepl("NTU", sensorDescription), 
...
distanceFromGoldenGate %>% 
  ...
  geom_sf(aes(fill = missingDays)) +
  geom_sf(data = data$cdecMetadata %>% 
            right_join(data$cdecDaily$OXYGEN %>% 
                         mutate(variable = ifelse(grepl("OXYGEN", sensorDescription), 
                                                  "dissolvedOxygen", "NA")) %>% 
                         distinct(stationId, variable), by = c("station" = "stationId")),
          color = "khaki",
          size = 2) +
...
(distanceFromGoldenGate %>% 
  ...
  reposition_legend('center', panel = 'panel-3-3')
...
distanceFromGoldenGate %>% 
  ...
  summarise(noData = factor(sum(is.na(value)) == 7, levels = c(T, F))) %>% 
  ggplot() +
  geom_sf(aes(fill = noData))
...
cdecCheck <- data$cdecDaily %>% 
  bind_rows() %>% 
  complete(obsDate = seq(min(obsDate, na.rm = T), max(obsDate, na.rm = T), by = "day"), 
           nesting(stationId, sensorNumber),
           fill = list(value = NA)) %>% 
  group_by(stationId, sensorNumber) %>% 
  summarise(
    totalDays = n(),
    missingDays = sum(is.na(value)),
    dateFirst = min(obsDate[!is.na(value)]),
    dateLast = max(obsDate[!is.na(value)]),
    daysInRange = as.integer(dateLast - dateFirst) + 1,
    missingDaysInRange = daysInRange - sum(!is.na(value)),
    .groups = "drop"
  ) %>% 
  # Bind to the metadata
  left_join(deltadata:::cdecMetadata %>%
              separate_wider_delim(dataAvailable, " to ", 
                                   names = c("dateStart", "dateEnd"), 
                                   too_few = "align_end"),
            by = c("sensorNumber", "stationId" = "gage")) %>%
  transmute(stationId, sensorNumber, dateFirst, dateLast,
            dateStart = if_else(mdy(dateStart) < as.Date("2011-08-23"), as.Date("2011-08-23"), mdy(dateStart)),
            dateEnd = if_else(dateEnd == "present", as.Date("2021-08-20"), mdy(dateEnd)),
            sameStart = dateFirst == dateStart,
            startDiff = as.integer(dateStart - dateFirst),
            sameEnd = dateLast == dateEnd,
            endDiff = as.integer(dateEnd - dateLast),
            missingDays, daysInRange, missingDaysInRange,
            sensorDescription)

# Redownload the data from the end of the last pulled data to end of interested data PER row...
checkLast <- cdecCheck %>% 
  filter(!sameEnd) %>%
  {
    lapply(1:nrow(.), function(rowIndex, maxAttempts = 3) {
      
      for (attempt in 1:maxAttempts) {
        result <- tryCatch({
          pullCDEC(station = .$stationId[rowIndex], 
                   sensor = .$sensorNumber[rowIndex], 
                   duration = .$durationFull[rowIndex], 
                   dateStart = .$lastDate[rowIndex] + 1, 
                   dateEnd = .$dateEnd[rowIndex])
        }, error = function(e) {
          if (grepl("cannot open the connection", e) && attempt < maxAttempts) {
            cat(sprintf("Attempt %d: Connection timed out, retrying\n", attempt))
            return(NULL)
            ...
            return(data.frame(
              stationId = .$stationId[rowIndex],
              sensorNumber = .$sensorNumber[rowIndex],
              duration = .$duration[rowIndex]
            ))
          }
        })
